...
// ignore: must_be_immutable
class TagStyleButton extends StatefulWidget with EditorStateReceiver {
  final EditorController controller;
  final IconData? icon;
  final double iconSize;
  final EditorIconThemeM? iconTheme;
  ...
  @override
  _TagStyleButtonState createState() => _TagStyleButtonState();
}

class _TagStyleButtonState extends State<TagStyleButton> {
  final GlobalKey _toolTipKey = GlobalKey();
  StreamSubscription? _refreshListener;
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isToggled = _getTagAttributeValue() != null;
    final pressedHandler = () => _openTagDialog(context);
  ...
  @override
  void didUpdateWidget(covariant TagStyleButton oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  void _openTagDialog(BuildContext context) {
    showDialog<dynamic>(
      context: context,
      builder: (ctx) {
        final tag = _getTagAttributeValue();
        final index = widget.controller.selection.start;

        var text;
        if (tag != null) {
          // text should be the tag's corresponding text, not selection
          final leaf =
              widget.controller.document.querySegmentLeafNode(index).leaf;
          if (leaf != null) {
            text = leaf.toPlainText();
        ...
        return TagStyleDialog(
          ...
          (value) {
        if (value != null) _tagSubmitted(value);
      },
    );
  }

  String? _getTagAttributeValue() {
    return widget.controller
        .getSelectionStyle()
        .attributes[AttributesM.tag.key]
        ?.value;
  }

  void _tagSubmitted(dynamic value) {
    // text.isNotEmpty && tag.isNotEmpty
    final text = (value as TagButtonM).text;
    final tag = value.tagValue.trim();
    var index = widget.controller.selection.start;
    var length = widget.controller.selection.end - index;

    if (_getTagAttributeValue() != null) {
      // text should be the tag's corresponding text, not selection
      final leaf = widget.controller.document.querySegmentLeafNode(index).leaf;
      if (leaf != null) {
        final range = getTagRange(leaf);
        index = range.start;
        length = range.end - range.start;
      }
    }
